...
Net<-visNetwork(nodes,edges,width="100%",main="Microbial Sharing Network between Carnivores of Betampona", submain="using DNA fingerprinting of E.coli", footer= "Rasambainarivo et al., in prep")%>%
  visGroups(groupname="Canis familiaris",color="darkblue",shape="square")%>%
  visGroups(groupname="Cryptoprocta ferox",color="forestgreen",shape="circle")%>%
  visGroups(groupname="Galidictis fasciata",color="gold",shape="circle")%>%
  visGroups(groupname="Galidia elegans",color="darkred",shape="circle")%>%
  visGroups(groupname="Felis catus",color="red",shape="star")%>%
  visLegend(position="left",main="Species",stepY = 200)
#set layout of network
Net<-Net%>%visPhysics(maxVelocity = 40)%>%
  visLayout(randomSeed = 123)

#highlight nearest nodes
# Net%>%visOptions(highlightNearest=TRUE)%>%
#   visOptions(selectedBy = "group")

Net%>%visOptions(manipulation=TRUE) %>% visPhysics(stabilization = FALSE)
